...
const receiveOrder = async (req, res) => {
  try {
    ...
    if (!order) return res.status(404).json({ success: false, message: "Order not found" });
    if (order.receivedBy) return res.status(400).json({ success: false, message: "Order already received" });
...
const assignShipping = async (req, res) => {
  try {
    ...
    if (!order) return res.status(404).json({ success: false, message: "Order not found" });
    if (!order.receivedBy) return res.status(400).json({ success: false, message: "Order must be received first" });
    if (order.assignedTo) return res.status(400).json({ success: false, message: "Order already assigned" });
