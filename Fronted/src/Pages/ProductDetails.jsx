...
const ProductDetails = () => {
  ...
  const apiUrl = import.meta.env.VITE_API;
  const cartApiUrl = import.meta.env.VITE_APIs;
  // serverUrl is no longer needed for images since Cloudinary URLs are already complete
  // const serverUrl = import.meta.env.VITE_SERVER;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-1/2 w-full">
          <img
            src={product.image || "/placeholder.png"} // ✅ fixed: was `${serverUrl}/upload/${product.image}`
            alt={product.name}
            className="w-full h-full object-cover rounded-xl shadow-md"
          />
        </div>
        ...
        <div className="mt-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {similarProducts.map((p) => (
              ...
              >
                <img
                  src={p.image || "/placeholder.png"} // ✅ fixed: was `${serverUrl}/upload/${p.image}`
                  alt={p.name}
                  className="w-full h-40 object-cover rounded"
                />
                <h3 className="mt-2 font-semibold">{p.name}</h3>
